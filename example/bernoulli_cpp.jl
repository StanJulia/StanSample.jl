...
isdir(tmpdir) && rm(tmpdir; recursive=true)
sm = SampleModel("bernoulli", bernoulli_model, tmpdir);
rc3 = stan_sample(sm, true, false;
    use_cpp_chains=true, num_cpp_chains=2, num_julia_chains=2, data);
...
isdir(tmpdir) && rm(tmpdir; recursive=true)
sm = SampleModel("bernoulli", bernoulli_model, tmpdir);
rc4 = stan_sample(sm, true, false; 
  use_cpp_chains=true, num_cpp_chains=4, num_julia_chains=4, data);
