...
if success(rc)
  chains = read_samples(sm, output_format=:mambachains)
  Mamba.describe(chains)

  gelmandiag(chains, mpsrf=true, transform=true) |> Mamba.showall
  gewekediag(chains) |> Mamba.showall
  heideldiag(chains) |> Mamba.showall
  #rafterydiag(chains) |> Mamba.showall
